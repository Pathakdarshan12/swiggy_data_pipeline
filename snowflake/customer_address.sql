...
CREATE OR REPLACE TABLE BRONZE.CUSTOMER_ADDRESS_BRZ (
    CUSTOMER_ADDRESS_BRZ_ID STRING(100),                    -- PRIMARY KEY AS STRING(100)
...
CREATE OR REPLACE TABLE SILVER.CUSTOMER_ADDRESS_SLV (
    ...
    CUSTOMER_ID INTEGER COMMENT 'CUSTOMER FK (SOURCE DATA)',                -- FOREIGN KEY REFERENCE AS STRING(100) (NO CONSTRAINT IN SNOWFLAKE)
    FLAT_NO STRING(100),
    HOUSE_NO STRING(100),
    FLOOR_NO STRING(100),
    BUILDING STRING(100),
...
CREATE OR REPLACE TABLE GOLD.DIM_CUSTOMER_ADDRESS (
    ...
    CUSTOMER_ID STRING(100) COMMENT 'CUSTOMER FK (SOURCE SYSTEM)',                            -- SURROGATE KEY FROM CUSTOMER DIMENSION (FOREIGN KEY)
...
BEGIN
    ...
    '
        COPY INTO TEMP_CUSTOMER_ADDRESS_LOAD (
            CUSTOMER_ADDRESS_BRZ_ID, CUSTOMER_ID, FLAT_NO, HOUSE_NO, FLOOR_NO, BUILDING,
            LANDMARK, LOCALITY, CITY, STATE, ZIPCODE, COORDINATES, PRIMARYFLAG, ADDRESSTYPE
        )
        FROM (
            SELECT
                $1::STRING AS CUSTOMER_ADDRESS_BRZ_ID,
    ...
    -- Get row count
    v_rows_inserted:= (SELECT COUNT(*) FROM BRONZE.CUSTOMER_ADDRESS_BRZ WHERE BATCH_ID = :p_batch_id);
...
BEGIN
    ...
    SELECT
        ...
        BATCH_ID,
        'BATCH_DUPLICATE',
        'Duplicate CUSTOMER_ID found in same batch'
    FROM BRONZE.CUSTOMER_ADDRESS_BRZ
    WHERE BATCH_ID = :P_BATCH_ID
    QUALIFY COUNT(*) OVER (
        PARTITION BY CUSTOMER_ADDRESS_BRZ_ID, BATCH_ID
    ) > 1;
    ...
    USING (
        ...
        FROM (
            SELECT
                TRY_CAST(B.CUSTOMER_ADDRESS_BRZ_ID AS INT) AS CUSTOMER_ADDRESS_SLV_ID,
                ...
                B.BATCH_ID,
                ROW_NUMBER() OVER (
                    PARTITION BY B.CUSTOMER_ADDRESS_BRZ_ID
            ...
            WHERE B.BATCH_ID = :P_BATCH_ID
              ...
              AND NOT EXISTS (
                    ...
                    WHERE S.CUSTOMER_ADDRESS_SLV_ID = TRY_CAST(B.CUSTOMER_ADDRESS_BRZ_ID AS INT)
              )
        )
        WHERE RN = 1
    ) AS SRC
    ON TGT.CUSTOMER_ADDRESS_SLV_ID = SRC.CUSTOMER_ADDRESS_SLV_ID

    WHEN NOT MATCHED THEN
        INSERT (
            CUSTOMER_ADDRESS_SLV_ID, CUSTOMER_ID, FLAT_NO, HOUSE_NO,
        ...
        VALUES (
            SRC.CUSTOMER_ADDRESS_SLV_ID, SRC.CUSTOMER_ID, SRC.FLAT_NO,
...
$$
DECLARE
    V_TS TIMESTAMP_TZ := CURRENT_TIMESTAMP();
    v_rows_deleted INTEGER DEFAULT 0;
    v_rows_inserted INTEGER DEFAULT 0;
    v_rows_updated INTEGER DEFAULT 0;
BEGIN
    /*---------------------------------------------------------
      STEP 0: Deduplicate SILVER into TEMP TABLE
    ---------------------------------------------------------*/
    CREATE OR REPLACE TEMP TABLE TMP_CUSTOMER_ADDRESS_DEDUP AS
    SELECT *
    FROM (
        SELECT
            *,
            ROW_NUMBER() OVER (
                PARTITION BY CUSTOMER_ADDRESS_SLV_ID
    ...
    ---------------------------------------------------------*/
    UPDATE GOLD.DIM_CUSTOMER_ADDRESS TGT
    SET
        EFF_END_DATE = :V_TS,
        STATUS = FALSE,
        UPDATED_AT = :V_TS
    FROM TMP_CUSTOMER_ADDRESS_DEDUP SRC
    WHERE TGT.CUSTOMER_ADDRESS_ID = SRC.CUSTOMER_ADDRESS_SLV_ID
      AND TGT.STATUS = TRUE
      AND (
            COALESCE(TGT.CUSTOMER_ID,'')    <> COALESCE(TO_VARCHAR(SRC.CUSTOMER_ID),'')
    ...
    /*---------------------------------------------------------
      ...
      - Insert if NO active record exists OR values differ
    ---------------------------------------------------------*/
    ...
    SELECT
        ...
        TO_VARCHAR(SRC.CUSTOMER_ID),  -- Cast INTEGER to STRING
        SRC.FLAT_NO,
        SRC.HOUSE_NO,
        SRC.FLOOR_NO,
        SRC.BUILDING,
    ...
    WHERE NOT EXISTS (
        ...
        WHERE TGT.CUSTOMER_ADDRESS_ID = SRC.CUSTOMER_ADDRESS_SLV_ID
          AND TGT.STATUS = TRUE
          AND COALESCE(TGT.CUSTOMER_ID,'')    = COALESCE(TO_VARCHAR(SRC.CUSTOMER_ID),'')
